...
remarks on CRF:
It is sensitive to response labeling!!!
When i model with response_recode (i.e., A B C), instead of the original responses (v_n_chulai, etc.) the factor importance change and the mean AUC drops slightly. This is due to the fact that CRF is sensitive to the order of response levels, and hence the changed probabilities it assigns to responses. 
...
## multiclass AUC
library(pROC) 
multiclass.roc(PV_data$Response_recode, predict(multinom_model, type="prob"))
```
